...
目前正在完成 P1 固定实验反应堆核心切片，已经具备或通过验收的主要能力包括：
...
- 燃料组件耐久、玩家顶部换料端口交互、燃尽后的冷却乏燃料组件，以及逐列状态持久化。
- 工程师护目镜分层显示：仪表端口显示结构和全堆运行摘要，换料端口与控制棒驱动器显示对应列详情。
- NBT 版本迁移、服务器配置、JUnit 与 NeoForge GameTest 回归，以及离线 HTML 反应堆数值模拟器。

当前 P1 仍在补齐：重新成型后的既有流体管网恢复、Create 机械臂换料、燃料列与控制棒列维修、危险拆除、完全停机重置、融毁动作和完整 Ponder 教学。
...
开发者和自动化执行者开始工作前必须阅读 [`AGENTS.md`](AGENTS.md)。新增或修改的手写代码使用中文注释；核心文档、活动计划和 Git 历史由项目经理与项目所有者统一维护。
