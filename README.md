...
**损伤倍率进度（2026-09-08）：** 游戏端已采用可配置的两条线性倍率曲线，默认满损伤产热 2 倍、燃耗 3 倍，且燃耗倍率必须增长更快；`P1-BALANCE-02A` 已通过自动验收。HTML 模拟器仍是旧等倍率规则，待 `P1-BALANCE-02B` 同步，详见 [专项任务计划](docs/superpowers/plans/2026-09-08-damage-heat-burn-balance-plan.md)。
...
`P1-BALANCE-02B` 现在可以同步模拟器、参数显示和导出格式。事故线的下一道实施门仍是冻结 `P1-MELTDOWN-02` 事故产物表，包括高放射性废物的注册 ID、形态和数量、热复合冷却剂喷出量以及结构破坏范围；随后完成幂等事故执行器及依赖它的维护工作、完整 Ponder 教学和最终端到端验证。
