...
**已确认、待实现（2026-09-08）：** 损伤采用可配置的两条线性倍率曲线，默认满损伤产热 2 倍、燃耗 3 倍，燃耗倍率必须增长更快。游戏端与模拟器当前仍是旧等倍率规则；实施与验收见 [P1-BALANCE-02 任务计划](docs/superpowers/plans/2026-09-08-damage-heat-burn-balance-plan.md)。下列既有验收不包含该变更。
...
P1 新增的损伤倍率需求已具备实施条件，按 `P1-BALANCE-02A` 游戏端、`P1-BALANCE-02B` 模拟器顺序验收。事故线的下一道实施门仍是冻结 `P1-MELTDOWN-02` 事故产物表，包括高放射性废物的注册 ID、形态和数量、热复合冷却剂喷出量以及结构破坏范围；随后完成幂等事故执行器及依赖它的维护工作、完整 Ponder 教学和最终端到端验证。
